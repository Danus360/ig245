...
import os
ban = ["436363464567"]
admin = ["264806304609075200","276071010808233985"]
...
async def on_message(message):
    ...
    if message.content.lower().startswith("ig!cookie"):
        emb=discord.Embed(title=":cookie: ", description = 'Requested by ' + message.author.name, color=0xd13bff)
        await client.send_message(message.channel, embed=emb)

    if message.content.lower().startswith("ig!readzattext"):
        f = open('Texts/Info.txt', 'r')
        content = f.read()
        emb=discord.Embed(title=content, color=0xd13bff)
        await client.send_message(message.channel, embed=emb)
    ...
    if message.content.lower().startswith("ig!suggest"):
        if message.author.id != ban:
            ...
            emb=discord.Embed(title="You have been blocked from sending suggestions! And a report has been sent to Discord.. :( ", color=0xd13bff)
            await client.send_message(message.channel, embed=emb)


    if message.content.lower().startswith("dn!qwerty"):
        if message.author.id == "264806304609075200" or "276071010808233985":
            args = message.content.split(" ")
            mesege = "%s" % (" ".join(args[2:]))
            await client.send_message(client.get_channel((args[1])), mesege)
        else:
            ...
            emb=discord.Embed(title="Error: Permission Not Granted", color=0xd13bff)
    ...
    if message.content.lower().startswith("ig!ping"):
        emb=discord.Embed(title='Pong', color=0xd13bff)
        await client.send_message(message.channel, embed=emb)    
    ...
    if message.content.lower().startswith("ig!flip"):
        cn=['Heads', 'Tails']
        emb=discord.Embed(title=random.choice(cn), color=0xffca38)
        await client.send_message(message.channel, embed=emb)

    if message.content.lower().startswith("ig!dice"):
        nmbrs= ['1','2','3','4','5','6']
        emb=discord.Embed(title=random.choice(nmbrs), color=0xffca38)
        await client.send_message(message.channel, embed=emb)   
    ...
    if message.content.lower().startswith("ig!8ball"):
        ...
        emb=discord.Embed(title=random.choice(bl), color=0xffca38)
        await client.send_message(message.channel, embed=emb)
